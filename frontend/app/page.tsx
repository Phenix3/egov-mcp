      ...
      <div className="p-5 border-b border-zinc-100">
        ...
        <div className="space-y-2 text-[11px] text-zinc-500">
          {[
            ...
            [Database, "Banque Mondiale (API temps réel)"],
          ].map(([Icon, label]: any) => (
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col">
      {/* ── Header ── */}
      <header className="h-[57px] bg-white border-b border-zinc-200 px-4 sm:px-5 flex items-center justify-between sticky top-0 z-30 shrink-0">
        <div className="flex items-center gap-2 sm:gap-2.5">
          {/* Mobile sidebar toggle */}
          <button
            onClick={() => setSidebarOpen(true)}
            className="lg:hidden p-1.5 rounded-md hover:bg-zinc-100 text-zinc-600 transition-colors"
            aria-label="Ouvrir le menu de référence"
          >
          ...
          <span className="text-zinc-300 select-none hidden sm:block">·</span>
          <span className="text-zinc-500 text-sm hidden sm:block">Guichet Fiscal PME Cameroun</span>
          <span className="px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-violet-50 text-violet-700 border border-violet-200 rounded-md">
            MCP
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 sm:gap-4 text-xs text-zinc-500">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-zinc-400 text-[11px] sm:text-xs">Édition 2026</span>
          </div>
        </div>
      </header>
      ...
      <div
        className={`fixed left-0 top-0 h-full w-[280px] bg-white border-r border-zinc-200 z-50 flex flex-col overflow-y-auto transition-transform duration-300 ease-in-out lg:hidden ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="h-[57px] flex items-center justify-between px-4 border-b border-zinc-200 shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md bg-violet-600 flex items-center justify-center">
              <Building2 className="w-3 h-3 text-white" />
            </div>
            <span className="font-semibold text-zinc-900 text-sm">Référence fiscale</span>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
